...
window.__ModuleLoader__.load({
    ...
    `;
		...
		// Only these pending kinds earn a navigation dot; the shipped list filters
		// the same three out of the framework's pending-interaction snapshot.
		const visiblePendingKind = (kind) => kind === "approval" || kind === "plan-review" || kind === "question" ? kind : undefined;

		function Browser(props) {
			const { wide, expandSidebar, useSessions, useWorkspaces, useSessionPendingInteraction, open, startSession, forkSession, archiveSession, renameSession } = props;
			const sessionsState = useSessions((s) => s);
			const workspacesState = useWorkspaces((s) => s);
			// Pending interactions no longer ride on the Session snapshot: the
			// framework exposes them through this selector hook, whose snapshot is
			// keyed by Session id. Reading s.pendingInteraction reported nothing.
			...
			const renderRows = (ids, depth, lastChain) => {
				...
				for (let i = 0; i < n; i++) {
					...
					const dotClass = visiblePendingKind(pendingInteractions?.get(id)?.kind) !== undefined
						? " pending"
						: s.running ? " running"
		...
		// Required client services (cordis fiber inject). Declaring them is what
		// makes the framework wait for their providers before apply runs; the
		// target slot is still reached through slots.inject() below, because the
		// slot-declaring entry's own apply order is not constrained.
		//
		// An empty array here activated this fiber immediately — before the
		// shipped plugins had provided sessions/workspaces — and the undefined
		// guards then returned silently: the plugin became a no-op with no error
		// anywhere. Never read a required service without declaring it.
		...
		function apply(ctx) {
			...
			if (slots === undefined || sessions === undefined || workspaces === undefined || uiWorkspace === undefined) {
				throw new Error("dsh-wbtree: slots/sessions/workspaces/uiWorkspace services are unavailable after inject");
			}
